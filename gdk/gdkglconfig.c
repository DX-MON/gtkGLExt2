...
static gpointer gdk_gl_config_parent_class = NULL;
...
GType gdk_gl_config_get_type(void)
{
	...
	if (!type)
	{
		...
		type = g_type_register_static(G_TYPE_OBJECT, "GdkGLConfig", &type_info, G_TYPE_FLAG_ABSTRACT);
	}
...
static void gdk_gl_config_class_init(GdkGLConfigClass *klass)
{
	...
	gdk_gl_config_parent_class = g_type_class_peek_parent(klass);
...
static GdkGLConfig *gdk_gl_config_new_ci(GdkScreen *screen, GdkGLConfigMode mode)
{
...
static GdkGLConfig *gdk_gl_config_new_by_mode_common(GdkScreen *screen, GdkGLConfigMode mode)
{
	GdkGLConfig *glconfig;

#define _GL_CONFIG_NEW_BY_MODE(__screen, __mode) \
	(((__mode) & GDK_GL_MODE_INDEX) ? gdk_gl_config_new_ci(__screen, __mode) : gdk_gl_config_new_rgb(__screen, __mode))
