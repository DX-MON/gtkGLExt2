...
static void gdk_gl_config_impl_x11_init(GdkGLConfigImplX11 *config);
static void gdk_gl_config_impl_x11_finalize(GdkGLConfigImplX11 *object);

static gpointer gdk_gl_config_x11_parent_class = NULL;
...
GType gdk_gl_config_impl_x11_get_type(void)
{
	...
	if (!type)
	{
		...
		type = g_type_register_static(GDK_TYPE_GL_CONFIG, "GdkGLConfigImplX11", &type_info, 0);
	}
...
static void gdk_gl_config_impl_x11_class_init(GdkGLConfigImplX11Class *klass)
{
	...
	gdk_gl_config_x11_parent_class = g_type_class_peek_parent(klass);
...
static void gdk_gl_config_impl_x11_finalize(GdkGLConfigImplX11 *impl)
{
	GObject *object = G_OBJECT(impl);
	...
	G_OBJECT_CLASS(gdk_gl_config_x11_parent_class)->finalize(object);
}
...
static GdkColormap *gdk_gl_config_get_std_rgb_colormap(GdkScreen *screen, XVisualInfo *xvinfo, gboolean is_mesa_glx)
{
	GdkDisplay *display;
	Display *xdisplay;
	int screen_num;
...
static GdkColormap *gdk_gl_config_setup_colormap(GdkScreen *screen, XVisualInfo *xvinfo, gboolean is_rgba, gboolean is_mesa_glx)
{
...
static GdkGLConfig *gdk_gl_config_new_common(GdkScreen *screen, const int *attrib_list)
{
	...
	impl->colormap = gdk_gl_config_setup_colormap(impl->screen, impl->xvinfo, is_rgba, impl->is_mesa_glx);

	GDK_GL_NOTE(MISC, g_message(" -- Colormap: visual id = 0x%lx", GDK_VISUAL_XVISUAL(impl->colormap->visual)->visualid));
 ...
 */
static XVisualInfo *gdk_x11_gl_get_xvinfo(Display *xdisplay, int screen_num, VisualID xvisualid)
{
...
static GdkGLConfig *gdk_x11_gl_config_new_from_visualid_common(GdkScreen *screen, VisualID xvisualid)
{
	GdkGLConfig *glconfig;
	GdkGLConfigImplX11 *impl;
