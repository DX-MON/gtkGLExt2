...
static void gtk_gl_widget_realize(GtkWidget *widget, GLWidgetPrivate *private);
static gboolean gtk_gl_widget_configure_event(GtkWidget *widget, GdkEventConfigure *event, GLWidgetPrivate *private);
static void gtk_gl_widget_size_allocate(GtkWidget *widget, GtkAllocation *allocation, GLWidgetPrivate *private);
static void gtk_gl_widget_unrealize(GtkWidget *widget, GLWidgetPrivate *private);
static void gtk_gl_widget_parent_set(GtkWidget *widget, GtkObject *old_parent, GdkColormap *colormap);
static void gtk_gl_widget_style_set(GtkWidget *widget, GtkStyle *previous_style, gpointer user_data);
...
static void gtk_gl_widget_realize(GtkWidget *widget, GLWidgetPrivate *private)
{
	...
	{
		glwindow = gdk_window_set_gl_capability(widget->window, private->glconfig, NULL);
		if (glwindow == NULL)
		{
		...
		if (private->unrealize_handler == 0)
			private->unrealize_handler = g_signal_connect(G_OBJECT(widget), "unrealize", G_CALLBACK(gtk_gl_widget_unrealize), private);
	}
...
static void gtk_gl_widget_parent_set(GtkWidget *widget, GtkObject *old_parent, GdkColormap *colormap)
{
	...
	if (gtk_widget_get_toplevel(toplevel) && !gtk_widget_get_realized(toplevel))
	{
		GTK_GL_NOTE(MISC, g_message(" - Install colormap to the top-level window."));
...
static void gtk_gl_widget_style_set(GtkWidget *widget, GtkStyle *previous_style, gpointer user_data)
{
	...
	if (gtk_widget_get_realized(widget))
	{
		GTK_GL_NOTE(MISC, g_message(" - window->bg_pixmap = %p", ((GdkWindowObject *)(widget->window))->bg_pixmap));
...
static void gl_widget_private_destroy(GLWidgetPrivate *private)
{
	...
	g_clear_object(&private->glconfig);
	g_clear_object(&private->share_list);
...
gboolean gtk_widget_set_gl_capability(GtkWidget *widget, GdkGLConfig *glconfig, GdkGLContext *share_list, gboolean direct, int render_type)
{
	...
	if (_gtk_gl_widget_install_toplevel_cmap)
	{
		...
		g_signal_connect(G_OBJECT(widget), "parent_set", G_CALLBACK(gtk_gl_widget_parent_set), colormap);
	...
	g_signal_connect_after(G_OBJECT(widget), "style_set", G_CALLBACK(gtk_gl_widget_style_set), NULL);
	...
	g_object_set_qdata_full(G_OBJECT(widget), quark_gl_private, private, (GDestroyNotify)gl_widget_private_destroy);

	/*
	 * Connect signal handlers to realize OpenGL-capable widget.
	 */

	g_signal_connect(G_OBJECT(widget), "realize", G_CALLBACK(gtk_gl_widget_realize), private);

	/* gtk_drawing_area sends configure_event when it is realized. */
	g_signal_connect(G_OBJECT(widget), "configure_event", G_CALLBACK(gtk_gl_widget_configure_event), private);
	...
	g_signal_connect(G_OBJECT(widget), "size_allocate", G_CALLBACK(gtk_gl_widget_size_allocate), private);
...
GdkGLContext *gtk_widget_get_gl_context(GtkWidget *widget)
{
	...
	if (private->glcontext == NULL)
		private->glcontext = gtk_widget_create_gl_context(widget, private->share_list, private->direct, private->render_type);
